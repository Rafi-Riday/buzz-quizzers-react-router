...
const Option = ({ optionInfo }) => {
    ...
    return (
        <label onClick={(e) => { e.preventDefault(); successToast(); setIsActiveOption(optionId) }} htmlFor={optionId} className={`${isActiveOption === optionId && 'bg-info text-black'} transition-all duration-150 border border-info rounded-xl text-sm font-extrabold p-3 pr-4 cursor-pointer flex items-center gap-2`}>
